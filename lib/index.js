...
function runToggle(script, enabled) {
  return new Promise((resolve) => {
    ...
    const args = ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', target, enabled ? '-On' : '-Off']
...
function apply(ctx, config) {
  ...
  try {
    ...
    // Reconcile once at load so the on-disk state follows the stored setting,
    // then on every committed change.
    sync(readSection() || { enabled: true })
    ctx.effect(
